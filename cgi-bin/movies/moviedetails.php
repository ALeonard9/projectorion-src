...
if ($_SESSION['usergroup'] == 'User' or $_SESSION['usergroup'] == 'Admin') {
    ...
    $metricssql =  "SELECT count(*) as completed_movies  FROM orion.g_user_movies where user_id = ".$user_id." AND completed = 1";
    $metricquery = $db->query($metricssql);
    $metrics = $metricquery->fetch(PDO::FETCH_ASSOC);
       
    echo "<div class='col-md-12'><a href='moviedetails.php?id=".$id."' class='fixed_middle_right' ><span class='glyphicon glyphicon-refresh'></span></a></div>
            <div class='col-md-6'>
              <a target='_blank' href='http://www.imdb.com/title/".$item['imdb']."'><img src='".$item['poster_url']."' class='img-rounded img-responsive center-block' style='margin-bottom:10px'></a>
