...
if ($_SESSION['usergroup'] == 'User' or $_SESSION['usergroup'] == 'Admin') {
    ...
    $sql = "SELECT v.title, v.country_code, g.g_id, g.rank, g.g_first, g.completed, g.notes FROM countries v, g_user_countries g WHERE v.id = " . $id . " AND v.id = g.countries_id AND g.user_id = " . $user_id . ";";
    $query = $db->query($sql);
    ...
    echo "<div class='col-md-12'><a href='countrydetails.php?id=".$id."' class='fixed_middle_right' ><span class='glyphicon glyphicon-refresh'></span></a></div>
            ...
            <div class='col-md-6'>
              <label>Your Ranking: ".$item['rank']."/".$metrics['completed']."</label></br>
              <label>First Visited: </label><input type='datetime-local' id='".$item['g_id']."'
